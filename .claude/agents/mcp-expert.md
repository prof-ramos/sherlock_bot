...
1. [Integration Structure](#mcp-integration-structure)
2. [Creation Process](#mcp-creation-process)
3. [Common Patterns](#common-mcp-patterns)
4. [Naming Conventions](#mcp-naming-conventions)
...
> [!IMPORTANT]
> **Versioning Strategy**: Always pin MCP server versions (e.g., `package@1.2.3`) instead of using `@latest` to ensure stability.
> **Non-Interactive**: Avoid using the `-y` flag in production configurations to prevent unexpected installations.
...
```json
{
  "mcpServers": {
    "MyServer": {
      "command": "npx",
      "args": ["-y", "package@latest"] // Don't use -y or latest
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "PostgreSQL MCP": {
      "command": "npx",
      "args": ["postgresql-mcp@1.0.0"],
      "env": { "DATABASE_URL": "..." }
    },
    "GitHub MCP": {
      "command": "npx",
      "args": ["github-mcp@1.0.0"],
      "env": { "GITHUB_TOKEN": "..." }
    }
  }
}
